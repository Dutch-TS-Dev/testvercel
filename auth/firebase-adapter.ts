...
import { Firestore, doc, getDoc, setDoc, updateDoc, collection, query, where, getDocs, deleteDoc } from "firebase/firestore";
...
export function FirebaseAdapter(db: Firestore): Adapter {
  return {
    ...
    async createSession(session) {
      ...
      const created = await getDoc(doc(db, "nextauth_sessions", session.sessionToken));
    ...
    async getSessionAndUser(sessionToken) {
      const sessionSnap = await getDoc(doc(db, "nextauth_sessions", sessionToken));
    ...
    async updateSession(session) {
      ...
      const updated = await getDoc(doc(db, "nextauth_sessions", session.sessionToken));
